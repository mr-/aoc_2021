...
sol2 =
  let p = Program puzzleInput 0 0
      initialHull = fromList [((0, 0) :: Position, 1 :: Integer)]
      (Just (_, _, _, hull)) = iterateAlt doThing (p, (0, 0), (0, 1), initialHull)
   in hull
...
ppHull :: Hull -> String
ppHull hull =
  let hullList = toList hull
      xs = [fst $ fst p | p <- hullList]
      ys = [snd $ fst p | p <- hullList]
      (minX, maxX) = (minimum xs, maximum xs)
      (minY, maxY) = (minimum ys, maximum ys)
      c x y = if member (x, y) hull then hull ! (x, y) else 0
      tr 0 = ' '
      tr 1 = '#'
      tr x = error $ show ("translate", x)
   in unlines $ [[tr $ c x y | x <- [minX .. maxX]] | y <- reverse [minY .. maxY]]
