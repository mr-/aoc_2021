...
solve1 :: [[String]] -> Int
solve1 wires =
  let p1 = toPoints (toInstructions $ wires !! 0)
      p2 = toPoints (toInstructions $ wires !! 1)
      p1Set = S.fromList p1
      p2Set = S.fromList p2
      intersections = S.toList $ p1Set `S.intersection` p2Set
      sorted = sortBy (\(a, b) (c, d) -> (abs a + abs b) `compare` (abs c + abs d)) intersections
      (a, b) = head sorted
   in abs a + abs b
...
solve2 wires =
  let p1 = toPoints (toInstructions $ wires !! 0)
      ...
      p2Set = S.fromList p2
      intersections = S.toList $ p1Set `S.intersection` p2Set
      steps = map (\p -> (fromJust $ elemIndex p p1, fromJust $ elemIndex p p2)) intersections
      sorted = sortBy (\(a, b) (c, d) -> (abs a + abs b) `compare` (abs c + abs d)) steps
      (a, b) = head sorted
   in 2 + a + b -- 2 + to account for the two missing (0,0)s

toPoints :: [Instruction] -> [Coord]
toPoints = toPoints' (0, 0)

toPoints' :: Coord -> [Instruction] -> [Coord]
toPoints' initial [] = []
toPoints' initial (i : is) =
  let (newInitial, points) = singleToPoints initial i
      after = toPoints' newInitial is
   in points ++ after

singleToPoints :: Coord -> Instruction -> (Coord, [Coord])
singleToPoints start (R c) = (start ^+^ (c, 0), [start ^+^ (x, 0) | x <- [1 .. c]])
singleToPoints start (L c) = (start ^+^ (- c, 0), [start ^+^ (- x, 0) | x <- [1 .. c]])
singleToPoints start (U c) = (start ^+^ (0, c), [start ^+^ (0, y) | y <- [1 .. c]])
singleToPoints start (D c) = (start ^+^ (0, - c), [start ^+^ (0, - y) | y <- [1 .. c]])
...
toInstruction ('U' : xs) = U (read xs)
toInstruction ('D' : xs) = D (read xs)
toInstruction e = error $ "WTF" ++ e
